    ...
    <div className='group relative flex items-center gap-4 mt-2 p-3 rounded-lg hover:bg-gray-100/60 transition-all duration-200'>
        ...
        <div className='flex items-center gap-2'>
            {!hideDeleteBtn && (
                <button 
                    onClick={onDelete}
                    className='text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all duration-200 cursor-pointer p-1 hover:bg-red-50 rounded'
                >
                    <Trash2 size={18} />
                </button>
            )}
            ...
            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-md ${getAmountStyles()}`}>
                <h6 className='text-xs font-medium'>
                    {type === 'income' ? '+' : '-'} ${odinNumberSeparator(amount)}
                </h6>
                {type === 'income' ? (
                    <TrendingUp size={15} />
                ):(
